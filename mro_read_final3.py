...
def remove_file_to(oldPath,newPath):
    f = glob.iglob(oldPath+"\*.xls")
...
if __name__ == "__main__":
    ...
    print('生成的是解析MRO名称的Excel文件！')
    print('*'*20)
    read_MRO(filePath)
    print("请稍等.....程序正在努力跑-。-")
    remove_csv_File(filePath)
    print('将解析的文件移动到一个文件夹')
    newPath = input('解析文件的新的文件夹路径(例如:D:\文件夹)')
    remove_file_to(filePath,newPath)
    #os.system('pause')
